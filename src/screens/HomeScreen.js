...
export default function HomeScreen({ navigation, route }) {
  ...
  function clearForm() {
    setName(''); setPriceFormatted(''); setPriceRaw('');
    setBarcode(''); setEditingProductId(null);
    formSnapshot.current = null;
  }
  ...
  function handleEditProduct(product) {
    ...
    const formatted = product.priceFormatted || priceDigitsToFormatted(product.price);
    setPriceFormatted(formatted);
              ...
              <View style={styles.productCard}>
                ...
                <View style={styles.productInfo}>
                  ...
                  <Text style={styles.productPrice}>
                    {item.priceFormatted || priceDigitsToFormatted(item.price) || '—'}
                  </Text>
                  {item.barcode ? (
...
const styles = StyleSheet.create({
  ...
  // Card preto do formulário
  formCard: {
    backgroundColor: BLACK,
    borderRadius: 28,
    padding: 22,
    marginTop: 20, // encosta no header verde
    marginBottom: 28,
  },
  formCardTitle: {
    color: WHITE,
